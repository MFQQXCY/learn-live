...
public class JukeboxData {
  ...
  public static void main(String[] args) {
    List<Song> songList = new JukeboxData().getSongs();

    // find all the "Rock" songs
    List<Song> result = songList.stream().filter(item->item.getGenre().toLowerCase().contains("rock")).collect(Collectors.toList());
    System.out.println(result);


    // List all the genres
    List<String> allGenres = songList.stream().map(Song::getGenre).distinct().toList();
